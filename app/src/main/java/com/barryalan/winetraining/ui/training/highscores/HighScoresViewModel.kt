...
class HighScoresViewModel(application: Application) : BaseViewModel(application) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val queryValueListener: ValueEventListener = object : ValueEventListener {
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    ...
                    viewModelScope.launch(Dispatchers.IO) {
                        firebaseScores.map {
                            AppDatabase(getApplication()).scoreDao().insertReplace(it)
                        }

                        val scores = AppDatabase(getApplication()).scoreDao().getAllScores()

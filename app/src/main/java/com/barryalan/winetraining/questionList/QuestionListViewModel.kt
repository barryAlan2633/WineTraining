package com.barryalan.winetraining.questionList
...
class QuestionListViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun saveQuestion(question: Question) {
        viewModelScope.launch(Dispatchers.IO) {
            AppDatabase(getApplication()).appDao().insert(question)
        }
        getQuestions()
    }
    ...
    private fun initQuestions() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            AppDatabase(getApplication()).appDao().insert(*allQuestions.toTypedArray())
        }
    }
